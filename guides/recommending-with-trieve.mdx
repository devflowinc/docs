...
Trieve provides the ability to get recommended chunks similar to your data through the [get recommended chunks route](/api-reference/chunk/get-recommended-chunks) or through the [get recommended groups route](/api-reference/chunk-group/get-recommended-groups).
...
Trieve offers different recommendation strategies for users to choose from:
...
- **`positive_examples` and `positive_tracking_ids`**: Provide the IDs of chunks for which similar chunks will be recommended.
- **`negative_examples` and `negative_tracking_ids`**: Provide the IDs of chunks that should be used as negative examples to avoid similar content.
